...
class Worker(QtCore.QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.send_data_to_consumer.emit(
                self.data, self.black_frame, self.black_frame
            )
...
class CameraWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent, *args, data, worker: Worker, **kwargs):
        ...
        self.worker.send_frame.connect(
            self.update_frame, type=QtCore.Qt.QueuedConnection
        )
        self.worker.send_update_status.connect(self.update_status)
        self.worker.error_occurred.connect(self.handle_error)

        self.CamLabel = QtWidgets.QLabel()
        self.CamLabel.setSizePolicy(
